...
public class TodoControllerTests {
    ...
    public void shouldUpdateTodo() throws Exception {
        TodoResponseDto oldTodo = new TodoResponseDto(1L, "Existing Todo", "Description", false);
        TodoResponseDto newTodo =
                new TodoResponseDto(1L, "Updated Todo", "Updated Description", true);

        given(todoService.findById(1L)).willReturn(oldTodo);
        given(todoService.update(anyLong(), any(TodoRequestDto.class))).willReturn(newTodo);

        mockMvc.perform(
                        patch("/api/todos/v2/1")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        "{\"title\": \"Updated Todo\", \"description\": \"Updated"
                                                + " Description\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1L))
