 ...
 * 토큰 관련 로직 담당 클래스
 *
 * <p>(SRP 관점에서 "Util" 클래스는 썩 좋은 방법은 아니다 - less cohesive - 현실적이지만) 어떻게 개선할 수 있을까? 생각해보기
 *
 * <p>책은 이 클래스를 "Serializable" 구현체로 만들던데 - 이유 자세히 알아보기 작성하다보니 많은 메서드들이 이 버전에선 deprecated되었다 ->
 * 현대화하기! (완료)
 *
 * <p>access-refresh 방식으로 고치는 것도 해보자
 */
@Component
        ...
        return Jwts.builder()
                ...
                .signWith(
                        Keys.hmacShaKeyFor(
                                secret.getBytes(
                                        StandardCharsets
                                                .UTF_8))) // 이 부분 잘 살펴보기... (잘못 구현하기 쉬운 것 같다)
                .compact();
    }
